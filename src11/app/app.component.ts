...
import { Platform, Nav,NavController, Events, Loading, LoadingController, ToastController,AlertController} from 'ionic-angular';
...
//import { } from './../pages/profilestepup/profilestepup';
...
export class MyApp {
  ...
  @ViewChild('TabsPage') tabRef: TabsPage;
  private profileMe: Profile;
  private loading: Loading;
  private loadingShown: boolean = false;
  rtlSide: string = "left";
  public notificationPlayerIds;
  public dismissing:boolean=false;
  public lastBack:any;
  public spamming:boolean=false;
  ...
  constructor(@Inject(APP_CONFIG) private config: AppConfig, public platform: Platform,
    ...
    private loadingCtrl: LoadingController, private toastCtrl: ToastController,public alertController: AlertController,public app: App) {
    ...
    //alert("ddd "+ alert(this.app.getActiveNav()));
   
   
    /*this.platform.ready().then(() => {
        this.platform.registerBackButtonAction(() => {
        console.log(this.app.getActiveNav());
        console.log(this.app.getActiveNav()._views[0]);
        console.log(this.app.getActiveNav()._views[0].component.name);
     if (this.app.getActiveNav()._views[0].component.name=="ReviewPage" || this.app.getActiveNav()._views[0].component.name=="AccountPage" || this.app.getActiveNav()._views[0].component.name=="ChatslistPage" || this.app.getActiveNav()._views[0].component.name=="NotificationsPage" || this.app.getActiveNav()._views[0].component.name=="RequestsPage" || this.app.getActiveNav() == null) {
        // getActiveNav is being deprecated so, fetch the first available nav.
        //alert("name "+this.app.getActiveNav()._views[0].component.name);
   ...
   platform.ready().then(() => {
    this.platform.registerBackButtonAction(() => {
        ...
        console.log("len "+this.nav.length());
      ...
      if (this.backButtonPressedOnceToExit) {
        ...
        let toast = this.toastCtrl.create({
          message: 'Apăsați din nou pentru a ieși',
          duration: 2000,
          position: 'bottom'
        });
      ...
      this.translate.get(["checking_profile", "something_wrong"]).subscribe(values => {
        ...
        this.clientService.getProfile().subscribe(res => {
          ...
          if (res.categories && res.categories.length) {
            ...
            this.nav.push(MyprofilePage, {send: this.sendby} );
          }
        }, err => {
