...
export class Upload_portfolioPage {
  ...
  saveFolio() {
    if (!this.imageToUpload || !this.imageToUpload.length) {
      this.translate.get('err_add_folio_image').subscribe(value => this.showToast(value));
    } else if (this.linkPortfolio && this.linkPortfolio.length && !Helper.isValidURL(this.linkPortfolio)) {
      this.translate.get('err_add_folio_link').subscribe(value => this.showToast(value));
    } else {
      this.translate.get('adding_folio_link').subscribe(value => this.showToast(value));
      this.subscriptions.push(this.service.addMyPortfolio({ image_url: this.imageToUpload, link: this.linkPortfolio }).subscribe(res => {
        this.dismissLoading();
        this.navCtrl.pop();
      }, err => {
