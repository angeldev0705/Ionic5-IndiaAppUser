...
import { qrResponse } from '../models/qrResponse.model';
...
export class ClientService {
    ...
    public getslides(token :  string , playload : any) :  Observable<qrResponse>{
        const myHeaders = new HttpHeaders({ 'Accept': 'application/json', 'Content-Type': 'application/json' , 'Authorization': 'Bearer ' + token });
        return this.http.post<qrResponse>(this.config.apiBase + "api/admin/getFeatureSlides", JSON.stringify(playload),  { headers: myHeaders }).concatMap(data => {
            return Observable.of(data);
        });
    }

    public checkQrCode(token: string, QRmessage: any ) : Observable<qrResponse>{
        const myHeaders = new HttpHeaders({ 'Accept': 'application/json', 'Content-Type': 'application/json' , 'Authorization': 'Bearer ' + token });
        return this.http.post<qrResponse>(this.config.apiBase + "api/admin/updateWinner", JSON.stringify(QRmessage),  { headers: myHeaders }).concatMap(data => {
            return Observable.of(data);
        }); 
    }
    ...
    public updateLocation(profileRequest: any): Observable<Profile> {
        return this.http.put<Profile>(this.config.apiBase + "api/provider/updateLocation", JSON.stringify(profileRequest), { headers: this.myHeaders }).concatMap(data => {
            this.setupProfile(data);
            return Observable.of(data);
        });
    }

    public priceUpdate(profileRequest: any): Observable<Profile> {
        return this.http.put<Profile>(this.config.apiBase + "api/provider/updateCatePrice", JSON.stringify(profileRequest), { headers: this.myHeaders }).concatMap(data => {
            this.setupProfile(data);
            return Observable.of(data);
        });
    }
