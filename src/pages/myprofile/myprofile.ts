...
export class MyprofilePage {
  ...
  subCategoriesNew = new Array<Category>();
  ...
  save() {
    ...
    if(this.calledby){
      ...
      if(this.profile.price < 20 || this.profile.price > 500){
  ...
  private showinfo(){
    let alert = this.alertCtrl.create({
      ...
      subTitle: 'Puteți modifica prețul pentru fiecare subcutieery din fila profilului dvs.',
      buttons: [
        {
          text: 'OK',
          handler: () => {
            this.save();
            console.log('on alert click ok');
          }
        }
      ]
    });
  ...
  loadChildCategories() {
    ...
    for (let cat of this.profile.categories) parentCategoryIds.push(cat.id);
    this.subscriptions.push(this.service.categoryChildren(parentCategoryIds,1).subscribe(res => {
      console.log(res);
      ...
      // }
      console.log(cats);
      this.subCategoriesNew.length = 0;
      this.subCategoriesNew = this.subCategoriesNew.concat(cats);
      // for(let cat of this.subCategoriesNew){
      //   this.toggleSelection(cat);
      // }
      // this.dothis();
      this.profile.subcategories = this.subCategoriesNew;
      console.log(this.profile);
    }, err => {
