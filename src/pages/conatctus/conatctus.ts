...
export class ConatctusPage {
  ...
  constructor(private navCtrl: NavController, private service: ClientService, private callNumber: CallNumber,
    ...
    this.supportRequest = new SupportRequest(this.profileMe.user.name, this.profileMe.user.email, "");
  }
  ...
  submitSupport() {
    if (!this.supportRequest.message || !this.supportRequest.message.length) {
      this.translate.get("err_valid_support_msg").subscribe(value => this.showToast(value));
    } else {
      this.translate.get("supporting").subscribe(value => {
        this.presentLoading(value);
      });
      let subscription: Subscription = this.service.submitSupport(this.supportRequest).subscribe(res => {
        this.dismissLoading();
